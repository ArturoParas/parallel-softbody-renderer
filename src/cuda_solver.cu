...
__device__ __inline__ void update_iteration_device(
...
__global__ void update_kernel_device()
{
  ...
  for (int i = 0; i < d_params.intermediate_steps; i++) {
    update_iteration_device(t_idx, b_idx, d_off, d_idx);
    __syncthreads();
  }
}

__device__ __inline__ float3 update_iteration_shared(
...
__global__ void update_kernel_shared()
{
  int t_idx = threadIdx.x;
  int b_idx = blockIdx.x;
  int d_off = b_idx * blockDim.x;
...
double solver_update_device(GlobalConstants& h_params, float* h_curr_particles)
{
  ...
  update_kernel_device<<<h_params.num_blocks, h_params.particles_per_block>>>();
...
{
  const auto kernel_start = std::chrono::steady_clock::now();
  update_kernel_shared<<<h_params.num_blocks, h_params.particles_per_block>>>();
  const auto kernel_end = std::chrono::steady_clock::now();
  const double kernel_time = std::chrono::duration_cast<std::chrono::duration<double>>(kernel_end - kernel_start).count();
  cudaCheckError(cudaDeviceSynchronize());
  cudaCheckError(cudaMemcpy(h_curr_particles, h_params.curr_particles,
